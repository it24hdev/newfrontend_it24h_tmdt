...
@section('content')
    ...
    @if(request()->input('search'))
        <div class="d-flex justify-content-center">
            <p class="my-0">Tìm thấy <strong>{{$products->total()}}</strong> sản phẩm cho từ khoá <strong>{{request()->input('search')}}</strong></p>
        </div>
    @endif
    </div>
    @if(!empty($cat))
                    ...
                    <div class="border_slider">
                        <div class="owl-carousel owl-theme owl-loaded owl-drag" id="slider-show">
                            @foreach ($sliders as $item)
                                <a>
                                    <img class="owl-lazy" data-src="{{asset('upload/images/products/thumb/'.$item)}}"  alt="">
                                </a>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="component-product filter_item">
                    ...
                    <div class="p-img">
                        <a href="{{ route('detailproduct', $value->slug)}}">
                            <img attr="{{$value->thumb}}" class="filter_item_img" src="@if($value->thumb != "no-images.jpg" && $value->thumb){{asset('upload/images/products/thumb/'.$value->thumb)}}@else{{asset('upload/images/common_img/no-images.jpg')}}@endif"
                            alt="{{$value->name}}">
                        </a>
                    </div>
                    <div class="info_cpn">
