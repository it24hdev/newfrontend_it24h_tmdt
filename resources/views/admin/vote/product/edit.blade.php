...
@section('subcontent')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12">
            <!-- BEGIN: Form Layout -->
            <form action="{{route('vote.updateProduct',['id'=>$vote->id])}}" method="post" id="form-post">
                <div class="intro-y box p-5">
                    <div class="mt-3">
                        ...
                        <select name="product" data-placeholder="Chọn sản phẩm" class="tom-select w-full" id="crud-form-2">
                            @foreach($arrProduct as $index => $item)
                                <option value="{{$index}}"
                                    {{ (old('product') == $index || (old('product') == null && $vote->product_id == $index)) ? 'selected': false }}
                                >{{$item}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mt-3">
                        ...
                        <input id="crud-form-2" type="text" name="name_user" value="{{old('name_user') ?? $vote->name_user}}" class="form-control w-full" placeholder="Nhập họ tên">
                        @error('name_user')
                    ...
                    <div class="mt-3">
                        ...
                        <div class="mt-2">
                            <textarea class="form-control" rows="3" name="comment"
                                      placeholder="Nhập bình luận">{{old('comment') ?? $vote->comment}}</textarea>
                            @error('comment')
                            <span style="color:red">{{$message}}</span>
                            @enderror
                        </div>
