...
<div class="intro-y col-span-12">
    <form action="{{ route('recruit.update',[$edit->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="grid grid-cols-12 gap-x-5">
            <div class="col-span-12 md:col-span-6">
                ...
                <div class="form-group mb-4">
                    <label>Vị trí</label>
                    <input type="text" class="form-control" name='vacancies' value="{{old('vacancies') ?? $edit->vacancies}}" required>
                    @error('vacancies')<span style="color: rgb(239 68 68);">{{ $message }}</span>@enderror
                </div>
                <div class="form-group mb-4">
                    <label>Phân loại</label>
                    <input type="text" class="form-control" name='type' value="{{old('type') ?? $edit->type}}" required>
                    @error('type')<span style="color: rgb(239 68 68);">{{ $message }}</span>@enderror
                </div>
                <div class="form-group mb-4">
                    <label>Số lượng</label>
                    <input type="number" class="form-control" name='quantum' value="{{old('quantum') ?? $edit->quantum}}" required>
                    @error('quantum')<span style="color: rgb(239 68 68);">{{ $message }}</span>@enderror
                </div>
                <div class="form-group mb-4">
                    <label>Thu nhập</label>
                    <input type="text" class="form-control" name='income' value="{{old('income') ?? $edit->income}}" required>
                    @error('income')<span style="color: rgb(239 68 68);">{{ $message }}</span>@enderror
                </div>
                <div class="form-group mb-4">
                    <label>Hạn nộp</label>
                    <input type="date" class="form-control" name='deadline' value="{{date('Y-m-d', strtotime($edit->deadline))}}" required>
