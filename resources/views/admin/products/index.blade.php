            ...
            <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
                <form action="{{ route('products.index')}}" method="get"
                    ...
                    <select id="limit" name="limit"
                            class="form-select  sm:w-32 2xl:w-full mt-2 sm:mt-0 sm:w-auto box mr-3"
                    ...
                    <select id="orderby" name="orderby"
                        ...
                        <option value="category_id" {{request()->input('orderby') =='category_id' ? 'selected' : ''}}>
                            Danh mục
                        </option>
                        <option value="brand" {{request()->input('orderby') =='brand' ? 'selected' : ''}}>Thương hiệu
                        </option>
                        <option value="quantity" {{request()->input('orderby') =='quantity' ? 'selected' : ''}}>Số
                            lượng
                        </option>
                        <option value="price" {{request()->input('orderby') =='price' ? 'selected' : ''}}>Giá</option>
                        <option value="limit_amount" {{request()->input('orderby') =='limit_amount' ? 'selected' : ''}}>
                            Cảnh báo sl tồn
                        </option>
                        <option value="status" {{request()->input('orderby') =='status' ? 'selected' : ''}}>Trạng thái
                        </option>
                    </select>
                    ...
                    <div class="sm:w-42 2xl:w-full mt-2 sm:mt-0 sm:w-auto box">
                        <div class="w-full relative text-gray-700 dark:text-gray-300">
                            <input id="search" type="text" name="keywords"
                                   class="form-control w-full box pr-10 placeholder-theme-13" placeholder="Tìm kiếm..."
    ...
    display: contents;">
        ...
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            <table class="table table-report -mt-2 p-1">
                <thead>
                <tr>
                    <th class="text-center whitespace-nowrap w-28" style="padding: 0.25rem 0.25rem !important;">Ảnh sản
                        phẩm
                    </th>
                ...
                @foreach ($products as $product)
                    <tr id="{{ $product->id }}">
                                ...
                                <a href="javascript:;" data-toggle="modal"
                                   ...
                                   title="Chi tiết sản phẩm">
                                    ...
                                    @if ($product->thumb == \App\Models\Products::IMAGE)
                                        <img src="{{ asset('/upload/images/common_img/') . '/' . $product->thumb }}"
                                             style="object-fit: cover; object-position: 50% 0; width: auto;height: auto;">
                                    @else
                                        <img src="{{ asset('/upload/images/products/medium') . '/' . $product->thumb }}"
                                             style="object-fit: cover; object-position: 50% 0; width: auto;height: auto;">
                            @endif
                        </td>
                        <td style="padding: 0.25rem 0.25rem !important;">
                                ...
                                <a href="javascript:;" data-toggle="modal"
                                   ...
                                   title="Chi tiết sản phẩm">
                                    ...
                                    <div class="font-medium"
                                         style="overflow-y: hidden;overflow-x: clip;width: 120px;text-overflow: ellipsis;max-height: 70px; text-align: left;"> {{ $product->name }}</div>
                        </td>
                        <td style="padding: 0.25rem 0.25rem !important;">
                            @can('viewAny', \App\Models\Products::class)
                                <a href="javascript:;" data-toggle="modal"
                            ...
                            @can('viewAny', \App\Models\Products::class)
                                <a href="javascript:;" data-toggle="modal"
                                   ...
                                   title="Chi tiết sản phẩm">
                                    @endcan
                                    <div class="font-medium text-center"
                                         style="overflow-y: hidden;overflow-x: clip; width: 120px;text-overflow: ellipsis;max-height: 70px; text-align: left;">
                        ...
                        <td class="w-10" style="padding: 0.25rem 0.25rem !important;">
                            @if ($product->status == 0)
                                <div class="text-theme-6 text-center">
                                    <i data-feather="check-square" class="w-4 h-4 mr-2"></i>
                                </div>
                            @else
                                <div class="text-theme-9 text-center">
                                    <i data-feather="check-square" class="w-4 h-4 mr-2"></i>
                                </div>
                            @endif
                        </td>
                            ...
                            <div class="flex justify-center items-center">
                                @can('update', App\Models\Products::class)
                                    <a href="{{ route('productsproperties.edit',['id' => $product->id]) }}"
                                       title="Thuộc tính sản phẩm" class="btn btn-sm btn-primary mr-2"><i
                                ...
                                @can('delete', App\Models\Products::class)
                                    <a title="Xóa" data-toggle="modal" data-value="{{ $product->id }}"
                                       ...
                                       class="btn btn-danger py-1 px-1 btn-delete">
                                        <i class="fa-solid fa-trash-can" style="padding: 1px"></i>
                                    </a>
                                @endcan
                            </div>
                        </td>
